...
func _close_main_menu():
	...
	tween_close_main_menu_ui.tween_property(main_menu_ui, "position", Vector2(0, 720), 0.5).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
...
func _show_level_menu_with_dissolve(level_menu):
	...
	tween_show_level_menu.tween_property(level_menu, "position", Vector2(0,0), 0.5).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
